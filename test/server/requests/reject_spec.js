...
describe('PUT /bathrooms/:id/requests/:requestId/rejected', () => {
  beforeEach(done => {
    Promise.all([
      User.remove({}),
      Bathroom.remove({})
  ...
  it('should return a bathroom with an array in requests', done => {
    api
      .put(`/api/bathrooms/${bathroomId}/requests/${requestId}/rejected`)
  ...
  it('the returned request should have a status of rejected', done => {
    api
      .put(`/api/bathrooms/${bathroomId}/requests/${requestId}/rejected`)
      .set('Authorization', `Bearer ${token}`)
      .end((err, res) => {
        expect(res.body.requests[0].status).to.eq('rejected');
        done();
      });
  });
});
